...
THREE.PlayerControls = function ( camera, player, domElement ) {
	...
	this.checkKeyStates = function () {
	    ...
	    if ( keyState[81] ) {
	        ...
	        this.player.position.y -= this.moveSpeed * Math.cos( this.player.rotation.y );
			//implement max depth to cords 3
	        ...
	        this.player.position.z += this.moveSpeed * Math.sin( this.player.rotation.z );

	        this.camera.position.y -= this.moveSpeed * Math.cos( this.player.rotation.y );
	        this.camera.position.z += this.moveSpeed * Math.sin( this.player.rotation.z );
	    ...
	    fbDatabase.child( "Players/" + playerID + "/orientation" ).update({
	    	position: {
	...
	function onMouseMove( event ) {
		...
		if ( state === STATE.ROTATE ) {
			...
			// mouse look to up change 0 to sth else if needed to look on y axis
			scope.rotateUp( 0 * Math.PI * rotateDelta.y / PIXELS_PER_ROUND * scope.userRotateSpeed );
	...
	// mouse scroll to see objects closer or father away
